...
export default function GarbageCollectorRoutes() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const results = await Promise.allSettled(keys.map(fetchRoutesForDate));
      if (!active) return;
      const next = {};
      const errors = [];
  ...
  const displayRoutes = useMemo(() => {
    ...
    return source
      .map((route) => {
        const start = route.start_time ? String(route.start_time).slice(0, 5) : '';
        const end = route.end_time ? String(route.end_time).slice(0, 5) : '';
        return {
          id: route.id,
          barangayName: route.barangay_name || route.name || 'Route',
          date: route.date || selectedDateKey,
          timeRange: start && end ? `${start} - ${end}` : start || end || '—',
          status: route.status,
          team_id: route.team_id,
          plate_num: route.plate_num,
          formattedDate: formatPrettyDate(route.date || selectedDateKey),
        };
      })
    ...
    <div className="p-6">
      ...
      <div className="grid gap-4">
        {displayRoutes.map((route) => (
          <div
            key={route.id}
            className="bg-green-50 rounded-xl shadow border border-green-100 px-5 py-4 overflow-hidden cursor-pointer hover:shadow-lg transition-shadow"
          ...
          >
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                ...
                <h3 className="text-lg font-semibold text-gray-900">{route.barangayName}</h3>
            ...
            <div className="mt-4 grid gap-2 text-sm text-gray-700">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">{route.timeRange || '—'}</span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-sm flex items-center justify-center px-4 py-8">
          <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-start justify-between gap-4 p-4 border-b border-gray-100">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">{mapRoute?.barangayName || 'Route Map'}</h2>
                <p className="text-sm text-gray-600">
                  {mapRoute?.formattedDate || mapRoute?.date}{mapRoute?.timeRange ? ` • ${mapRoute.timeRange}` : ''}
                </p>
              </div>
              <button
