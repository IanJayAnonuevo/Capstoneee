...
import { MdAccessTime, MdCalendarToday, MdRoute, MdLocalShipping } from 'react-icons/md';
...
export default function TruckDriverHome() {
  ...
  if (currentHour >= AM_START_HOUR && currentHour < AM_END_HOUR) currentWindow = 'AM_TIME_IN';
  else if (currentHour >= AM_TIMEOUT_START && currentHour < AM_TIMEOUT_END) currentWindow = 'AM_TIME_OUT';
  else if (currentHour >= PM_START_HOUR && currentHour < PM_END_HOUR) currentWindow = 'PM_TIME_IN';
  else if (currentHour >= PM_TIMEOUT_START && currentHour < PM_TIMEOUT_END) currentWindow = 'PM_TIME_OUT';
  ...
  return (
    <div className="truck-driver-home flex-1 bg-gray-50 px-4 py-4">
