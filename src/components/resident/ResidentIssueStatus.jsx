...
import { FiAlertCircle, FiClock, FiCheckCircle, FiXCircle, FiCalendar, FiMapPin, FiTag, FiImage, FiUser } from 'react-icons/fi';
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm text-green-800 font-semibold">
                    {issue.status === 'closed' ? 'This issue has been closed.' : 'This issue has been resolved by the admin team.'}
                  </p>
                  {issue.resolved_at && (
...
export default function ResidentIssueStatus() {
  ...
  const fetchIssues = async () => {
    try {
      ...
      let endpoint = `get_user_issue_reports.php?user_id=${encodeURIComponent(userId)}`;
      if (filterStatus !== 'all') {
      ...
      if (response.data.status === 'success') {
        const normalizedIssues = (response.data.data || []).map((issue) => {
          const normalizedStatus = (issue.status || 'pending').toLowerCase();
      ...
      setError('Failed to load your issue reports. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center bg-gradient-to-br from-green-50 to-white py-8 px-4">
        <div className="text-center">
          ...
          <p className="text-green-700 font-medium">Loading your issue reports...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[80vh] bg-gradient-to-br from-green-50 to-white py-8 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">My Issue Reports</h1>
          <p className="text-gray-600">Track the status of your submitted issue reports</p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">
              {filterStatus === 'all'
                ...
                : `You don't have any ${filterStatus} issues at the moment.`}
            </p>
            <button
