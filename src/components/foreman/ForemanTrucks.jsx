...
export default function ForemanTrucks() {
    ...
    const fetchTrucks = async () => {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/get_equipment_status.php`, {
                headers: {
            ...
            if (data.success && data.data && data.data.trucks) {
                // Map the response to match expected format
                const mappedTrucks = data.data.trucks.map(truck => ({
                    truck_id: truck.truck_id,
                    plate_num: truck.plate_number || truck.plate_num,
    ...
    const updateTruckStatus = async (truckId, newStatus) => {
        try {
            ...
            const token = localStorage.getItem('access_token');
            const response = await fetch(`${API_BASE_URL}/update_truck_status.php`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            if (data.success) {
                // Use the normalized status from the API response
                const updatedStatus = data.new_status || newStatus;
                setTrucks(trucks.map(truck =>
                    truck.truck_id === truckId
                        ? { ...truck, status: updatedStatus }
                        : truck
                ));
                // Success - no alert needed, visual feedback is enough
            } else {
                console.error('Update failed:', data.message);
                alert('Failed to update truck status: ' + (data.message || 'Unknown error'));
                fetchTrucks();
            }
        } catch (error) {
                    ...
                    {filteredTrucks.map((truck) => (
                        ...
                        >
                            ...
                            <div className="relative">
                                <select
                                    value={(truck.status || 'available').toLowerCase().replace(/ /g, '_')}
                                    onChange={(e) => updateTruckStatus(truck.truck_id, e.target.value)}
                                    disabled={updatingStatus === truck.truck_id}
                                    className={`w-full px-3 py-2 rounded-lg border cursor-pointer text-sm font-medium ${getStatusColor(truck.status)} ${updatingStatus === truck.truck_id ? 'opacity-50' : 'hover:opacity-80'
                                        }`}
                                >
